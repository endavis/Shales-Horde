...
public class OrangeBlockLeather extends BackpackBlock {
    public OrangeBlockLeather(Settings settings) {
        super(settings);
    }
    @Override
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (player.isInSneakingPose()) {
            if (player.getStackInHand(hand).isEmpty()) {
                ...
                player.giveItemStack(BB_LeatherItemRegistry.LEATHER_ORANGE_ITEM.getDefaultStack());
            }
        } else {
