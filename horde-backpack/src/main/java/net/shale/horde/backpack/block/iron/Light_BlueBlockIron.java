...
public class Light_BlueBlockIron extends BackpackBlock {
    public Light_BlueBlockIron(Settings settings) {
        super(settings);
    }
    @Override
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (player.isInSneakingPose()) {
            if (player.getStackInHand(hand).isEmpty()) {
                ...
                player.giveItemStack(BB_IronItemRegistry.IRON_LIGHT_BLUE_ITEM.getDefaultStack());
            }
        } else {
