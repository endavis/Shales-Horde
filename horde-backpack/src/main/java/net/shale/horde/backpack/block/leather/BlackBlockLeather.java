...
public class BlackBlockLeather extends BackpackBlock {
    public BlackBlockLeather(Settings settings) {
        super(settings);
    }
    @Override
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (player.isInSneakingPose()) {
            if (player.getStackInHand(hand).isEmpty()) {
                ...
                player.giveItemStack(BB_LeatherItemRegistry.LEATHER_BLACK_ITEM.getDefaultStack());
            }
        } else {
