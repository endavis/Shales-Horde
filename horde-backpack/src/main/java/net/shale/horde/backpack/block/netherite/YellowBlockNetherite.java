...
public class YellowBlockNetherite extends BackpackBlock {
    public YellowBlockNetherite(Settings settings) {
        super(settings);
    }
    @Override
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (player.isInSneakingPose()) {
            if (player.getStackInHand(hand).isEmpty()) {
                ...
                player.giveItemStack(BB_NetheriteItemRegistry.NETHERITE_YELLOW_ITEM.getDefaultStack());
            }
        } else {
