...
public class BlueBlockNetherite extends BackpackBlock {
    public BlueBlockNetherite(Settings settings) {
        super(settings);
    }
    @Override
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (player.isInSneakingPose()) {
            if (player.getStackInHand(hand).isEmpty()) {
                ...
                player.giveItemStack(BB_NetheriteItemRegistry.NETHERITE_BLUE_ITEM.getDefaultStack());
            }
        } else {
