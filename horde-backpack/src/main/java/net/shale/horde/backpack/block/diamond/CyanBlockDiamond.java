...
public class CyanBlockDiamond extends BackpackBlock {
    public CyanBlockDiamond(Settings settings) {
        super(settings);
    }
    @Override
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (player.isInSneakingPose()) {
            if (player.getStackInHand(hand).isEmpty()) {
                ...
                player.giveItemStack(BB_DiamondItemRegistry.DIAMOND_CYAN_ITEM.getDefaultStack());
            }
        } else {
