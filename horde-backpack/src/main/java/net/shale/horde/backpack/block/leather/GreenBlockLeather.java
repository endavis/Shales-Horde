...
public class GreenBlockLeather extends BackpackBlock {
    public GreenBlockLeather(Settings settings) {
        super(settings);
    }
    @Override
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (player.isInSneakingPose()) {
            if (player.getStackInHand(hand).isEmpty()) {
                ...
                player.giveItemStack(BB_LeatherItemRegistry.LEATHER_GREEN_ITEM.getDefaultStack());
            }
        } else {
