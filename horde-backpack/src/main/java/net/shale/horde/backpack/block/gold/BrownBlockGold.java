...
public class BrownBlockGold extends BackpackBlock {
    public BrownBlockGold(Settings settings) {
        super(settings);
    }
    @Override
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (player.isInSneakingPose()) {
            if (player.getStackInHand(hand).isEmpty()) {
                ...
                player.giveItemStack(BB_GoldItemRegistry.GOLD_BROWN_ITEM.getDefaultStack());
            }
        } else {
