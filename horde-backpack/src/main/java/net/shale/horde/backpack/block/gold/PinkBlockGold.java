...
public class PinkBlockGold extends BackpackBlock {
    public PinkBlockGold(Settings settings) {
        super(settings);
    }
    @Override
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (player.isInSneakingPose()) {
            if (player.getStackInHand(hand).isEmpty()) {
                ...
                player.giveItemStack(BB_GoldItemRegistry.GOLD_PINK_ITEM.getDefaultStack());
            }
        } else {
